  ...
  Looper.prototype.ResetTime = function(){
    console.log( this.timer );
    console.log( this.timer.time );
    
   G.audio.mute.gain.setValueAtTime(0.01, G.audio.ctx.currentTime);
   G.audio.mute.gain.exponentialRampToValueAtTime(1, G.audio.ctx.currentTime+6);
    this.timer.value = 0;
    this.loopStartTime = this.timer.value;
    this.measureStartTime = this.timer.value;
  }
  ...
  Looper.prototype.addNoteHit = function( note , measure,location ){

      var hit = {
        callback: function(){ note.play(); },
        measureFrequency:4,
        measureOffset: measure,
        percents:location,
        duration:[0,100000000000] // forever
  ...
  Looper.prototype.addEveryNote = function( callback ){

    var percentages = []
    for( var i = 0; i < this.beatsPerMeasure; i++ ){
      percentages.push( i / this.beatsPerMeasure );
    }
    this.addHit( callback , {
